...
**The design-match queue is complete**, the Programs screen has shipped, **P1 (assigned programs now reach the client portal)** and **P2 (assign-modal parity for programs)** are done, and the **Dashboard has been rebuilt as a daily action queue**. Next work comes from the backlog below — recommended order: P3 (populate the empty planner/list columns), then the coach-side Training tab.
...
- **P3 — Real data in empty columns.** Render persisted **tags**; derive **Equipment** for list rows from the assigned workouts' exercises. (Live Sync stays decorative.) **Recommended next.**
