...
| Dashboard (daily action queue) | ✅ Done — rebuilt on a new `/coach/dashboard/overview` aggregate (`feat/dashboard-overview-api` + `feat/dashboard-revamp`) |

**The design-match queue is complete**, the Programs screen has shipped, **P1 (assigned programs now reach the client portal)** is done, and the **Dashboard has been rebuilt as a daily action queue**. Next work comes from the backlog below — recommended order: assign-modal parity (view/undo for programs), then populate the empty planner/list columns.
...
- Dashboard revamp (`feat/dashboard-overview-api` + `feat/dashboard-revamp`) — replaced the four vanity counters + recent-clients list with a **one-window daily action queue**. Backend: new `GET /api/v1/coach/dashboard/overview` (`DashboardOverviewService`) aggregating counts, a prioritised action queue (unanswered messages → overdue check-ins → expiring plans → no plan → onboarding), today's sessions/check-ins, roster + tier limit, subscription/trial and an activity feed. Built server-side because every signal was previously per-client only (~3 requests × roster from the browser); it now runs ~10 coach-scoped queries flat, using new `GROUP BY` aggregates (`findUnreadConversations`, `findLastCheckInPerClient`) and loading the roster once into a map. Overdue reuses `CheckInReminderScheduler`'s 7-day rule; the cap comes from `SubscriptionGate.clientLimitFor` (now public) so the reported and enforced limits can't drift. Old `/coach/dashboard` kept but `@Deprecated` — no contract break. `DashboardOverviewIntegrationTest` (16 cases incl. tenant isolation) green; full suite 276 green. Frontend: `lib/dashboard-api.ts` typed off the regenerated `types/api.ts`, `components/dashboard/*`, `/dashboard` added to `fullBleed`. The queue **groups by client** (one row per person, top-priority reason leading, others as secondary pills) — showing one client under several reasons read as noise. Also extracted the duplicated `Spark`/`Delta`/`ClientAvatar`/`StatusPill` into `components/ui/` (Clients + Inbox now import them) and replaced two hardcoded values with real data: the sidebar's fake `badge: 3` and the trial chip's copy. 5 new Playwright specs; 24/24 e2e, tsc + build clean.
  ...
  - Still hardcoded elsewhere: the "29 days left" banners in `ProgramListView.tsx` / `ProgramPlannerView.tsx` (P4) can now reuse `components/dashboard/TrialChip.tsx`.
...
- **P4 — Replace hardcoded trial UI.** 🟡 **Partly done** — the dashboard header now shows real trial state via `components/dashboard/TrialChip.tsx` (fed by `subscription.trialEndsAt` from the overview endpoint). The hardcoded "29 days left … / Upgrade" still sits in `ProgramListView`/`ProgramPlannerView`; point those at `TrialChip` (it needs the subscription prop, or a small shared hook over `GET /billing/status` so it works outside the dashboard).
- **P5 — Wire/scope remaining stubs.** `Explore Templates` → a program-template flow mirroring `workout_templates`; re-add per-day notes editing (`ProgramDay.notes` exists); decide on `Master Planner` / `Filter` / `Tags`. Also **"Save to Library"** on the planner card menu (`feat/program-calendar-card-menu`) currently only toasts — needs a backend to persist a day's workout as a reusable template/library entry.
