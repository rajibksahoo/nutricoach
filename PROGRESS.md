...
**The design-match queue is complete**, the Programs screen has shipped, **P1/P2/P4 are done**, the **Dashboard has been rebuilt as a daily action queue**, and the **revenue path (onboarding + legal pages + real Razorpay Checkout) has shipped**. Next work comes from the backlog below. The design-match queue and the unwired-slot list are now clear except for **Package / pkgEnd**, which needs a **product decision, not code** (a per-client billing model — today subscriptions are coach-tier). `Master Planner` used to sit beside it here; that button was deleted on 2026-09-22 rather than specified. The remaining engineering work is **P6, prod-only verification**, and filling in `nutricoach-web/lib/legal.ts` before taking a payment. **Before going live:** fill in the `REPLACE_ME` business details in `nutricoach-web/lib/legal.ts` and have the policy copy reviewed.
...
Messaging is **done**: `messaging/` module (entity/repo/service/mapper, `MessageController` at `/api/v1/messages/*` + `ClientMessagingController` at `/api/v1/portal/messages`, changeset `015-create-messages.xml`, `MessagingIntegrationTest`) and `InboxScreen` is wired via `lib/messaging-api.ts` (web PR #37). The original messaging proposal that lived here is obsolete and was removed. **WhatsApp notification on in-app send shipped 2026-09-22** (be #40, `ClientMessageNotifier`): a coach's message pings the client, throttled to one ping per thread the client has caught up on, with a 24h nudge for a cold thread. Note it has never actually been delivered — see GO-LIVE §4. Still open from the original proposal: mark-as-read endpoint (no controller exposes one), WATI inbound webhook, real-time delivery (polling/SSE).
...
- **P5 — Wire/scope remaining stubs.** ✅ **Done** — per-day notes, Filter/Tags and coach-owned templates shipped; see the log above. The last open item, `Master Planner`, closed 2026-09-22: it was never defined anywhere in the repo or the design bundle, so the button was **deleted** rather than specified (web #61, 0 references remain). If the feature is ever wanted it starts from a blank page, not from this stub.
